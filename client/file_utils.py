...
def show_processing_bar(current_value, all_value, line_txt=''):
    per = all_value / 100
    ...
    # "\r" 表回到行首
    equal_num = int(int_value / 10)
    space_num = 10 - equal_num
    processing_bar = f'\r{line_txt} [{"=" * equal_num}>{" " * space_num}] {float_value:.2f} %'
    print(processing_bar, end=end)
...
class Uploader(object):
    ...
    def upload_dir(self, dir):
        ...
        print(f'上传到服务器: {self.host}:{self.port}; 目录: {self.remote_save_dir} ')
        for file in files:
    ...
    def upload_file(self, file, file_relative_path=None):
        ...
        if not file_relative_path:
            filesplit = os.path.split(file) # ('c:/temp/myweb', 'myweb.api.dll')
        ...
        if self.tcp_client.recv(1024).decode(self.encoding) == 'ready_for_new':
            ...
            msg = f'uploading file: {file_relative_path}'
            print(msg)
...
def main():
    # files = find_files('D:\\.NET\\my\\Doc\\download\\temp\\my.site.api', include_strs=['.dll']) # 只要dll文件
    #files = find_files('D:\\.NET\\my\\Doc\\download\\temp\\my.site.api', exclude_strs=['.dll']) # 除了dll文件
    #files = find_files('D:\\.NET\\my\\Doc\\download\\temp\\my.site.api', include_strs=['my', 'site']) # 只要包含my和site的文件
    files = find_files('D:\\.NET\\my\\myapp\\myapp.api\\bin\\Release\\net5.0\\publish')
    i = 0
    for f in files:
        i += 1
        print(f'{i}: {f}')
    
    #Uploader('192.168.1.81', 8989, 'utf-8', 'D:/.NET/my/Doc/download/temp').upload('D:/others.projects/tools/zips/my.app.api.zip')
    #Uploader('192.168.1.81', 8989, 'utf-8', 'D:/.NET/my/Doc/download/temp/my.site.api', '.dll').upload('D:/others.projects/tools/zips/publish')
